    ...
    def train(self, data_loader, run_folder, epochs, test_A_file, test_B_file, batch_size=1, sample_interval=50):
        ...
        for epoch in range(self.epoch, epochs):
            for batch_i, (imgs_A, imgs_B) in enumerate(data_loader.load_batch()):
                ...
                # if g_loss on BA is below something, save the weight
                if g_loss[0] <= 1.5:
                    print("!!!! Got g_loss at epoch  " + str(self.epoch) + " " + str(g_loss[1]))
                    self.sample_images(data_loader, batch_i, run_folder, test_A_file, test_B_file)
                    self.combined.save_weights(os.path.join(run_folder, 'weights/weights-below-%d.h5' % (self.epoch)))
        ...
        for p in range(2):
            ...
            for i in range(r):
                for j in range(c):
                    ...
                    axs[i,j].axis('off')
                    cnt += 1
            fig.savefig(os.path.join(run_folder ,"images/below_%d_%d_%d.png" % (p, self.epoch, batch_i)))
            plt.close()
